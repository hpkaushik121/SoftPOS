...
public class CA {
    ...
    static {
        _initFromFile("raw/threeline_capks.json");
    }
    ...
    public static void addFromXmlFile(String fileName) {
        try {
            ...
            for (Object caObject : certificationAuthoritiesElement.getChildren()) {
                ...
                if (rid.length != 5) {
                    throw new RuntimeException("Unexpected RID length: " + rid.length + ". Length must be 5 bytes. RID=" + Util.prettyPrintHexNoWrap(rid));
                }
                    ...
                    if ("Name".equalsIgnoreCase(name)) {
                        ...
                        for (Object pkObject : caChildElement.getChildren()) {
                            ...
                            if (!Arrays.equals(hash, sha1ChecksumResult)) {
                                throw new RuntimeException("Checksum not correct for key index " + index + " for CA RID " + Util.prettyPrintHexNoWrap(ca.getRID()) + ". Expected " + Util.byteArrayToHexString(hash) + " but was " + Util.byteArrayToHexString(sha1ChecksumResult));
                            }
                            Date expirationDate = null;
                            try {
                                expirationDate = DATE_FORMAT.parse(expirationDateStr);
                            } catch (ParseException ex) {
                                throw new RuntimeException("Expiration date not valid. Must be in the format dd MMM yyyy, (where MMM is the english name of the month), but was: "+expirationDateStr);
                            }
