...
public class TerminalVerificationResults {
    ...
    TerminalVerificationResults(byte[] data) {
        if (data.length != 5) {
            throw new RuntimeException("TVR must be initialized with 5 bytes. Length=" + data.length);
        }
    ...
    public String parse(){
        return BytesUtils.bytesToString(toByteArray()).replace(" ","").trim();
    }
